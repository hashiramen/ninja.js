...
it('renders without crashing', () => {
  shallow(<DataTable data={[]} locale="da" options={{ rowsPerPage: 5 }} />);
});

it('renders 5 rows', () => {
  const wrapper = mount(<DataTable data={rows} locale="da" options={{ rowsPerPage: 5 }} />);

  expect(wrapper.find('tr').length).toBe(5);
});

it('filters rows based on input', () => {
  const wrapper = mount(<DataTable data={rows} locale="da" options={{ rowsPerPage: 5, search: true }} />);
